-- Migration to match comprehensive Smart Office Seating Planner specification
...
-- 7. Update assignments table to reference schedule_days
...
-- Note: We'll migrate assignment data after creating schedule structure
-- For now, keep existing schedule_assignments table until migration is complete
...
-- 9. Create RPC helpers for the frontend
CREATE OR REPLACE FUNCTION rpc.week_schedule(week_start DATE)
RETURNS TABLE (
  day_name TEXT,
  employee_id TEXT,
  seat_id TEXT,
...
CREATE OR REPLACE FUNCTION rpc.zone_summary(schedule_id UUID, day_name TEXT)
RETURNS TABLE (
  zone TEXT,
  total_seats INTEGER,
  occupied INTEGER,
...
CREATE OR REPLACE FUNCTION rpc.floor_occupancy(schedule_id UUID, day_name TEXT)
RETURNS TABLE (
  floor INTEGER,
  occupied BIGINT,
  total BIGINT
...
CREATE POLICY "Viewers can select assignments" ON public.assignments_new
  FOR SELECT USING (public.get_user_role() IN ('viewer', 'manager', 'admin'));
...
CREATE POLICY "Managers can insert assignments" ON public.assignments_new
  FOR INSERT WITH CHECK (public.get_user_role() IN ('manager', 'admin'));

CREATE POLICY "Managers can update assignments" ON public.assignments_new
  FOR UPDATE USING (public.get_user_role() IN ('manager', 'admin'));
...
CREATE POLICY "Admins can delete assignments" ON public.assignments_new
  FOR DELETE USING (public.get_user_role() = 'admin');
...
CREATE TRIGGER update_assignments_updated_at BEFORE UPDATE ON public.assignments_new
    FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE INDEX idx_assignments_schedule_day_id ON assignments_new(schedule_day_id);
CREATE INDEX idx_assignments_employee_id ON assignments_new(employee_id);
CREATE INDEX idx_assignments_seat_id ON assignments_new(seat_id);
